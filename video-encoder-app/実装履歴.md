...
## 2026-01-27 Git履歴のクリーンアップスクリプト作成

### 依頼内容
- `.gitignore` で指定されているファイルやフォルダを Git の全履歴から削除するためのコマンド（スクリプト）を作成。
- 現在プルリクエスト作業中のため、実行はせず作成のみ。

### 実装内容
- **[purge_ignored_history.ps1](../purge_ignored_history.ps1)** を作成。
    - プロジェクト内の全ての `.gitignore` を再帰的に読み取り、対象パスを自動抽出。
    - `git filter-repo` を利用した削除コマンドを生成。
    - PowerShell のコメントベースヘルプ (`Get-Help`) および `#region` に対応。
    - `-Force` 引数を付けない限り実行されず、プレビューのみを表示する安全設計。
...
## 2026-01-28 GitHub Actions ワークフロー修正
- .github/workflows/gh-pages.yml: トリガーブランチを `main` ブランチにも追加。`feature/static-github-pages` だけでなく、`main` ブランチへのプッシュでもGitHub Pagesデプロイが実行されるよう修正。これにより、メイン開発ブランチでの変更が自動デプロイされる。

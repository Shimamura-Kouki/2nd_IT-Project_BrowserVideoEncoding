# ブラウザ完結型動画エンコードアプリ - プログラム実装ガイド

このディレクトリには、WebCodecs APIを使用したブラウザ完結型の動画エンコードアプリケーションの実装が含まれています。
...
4. **容量ターゲットエンコード**: 指定サイズ（Discord 8MB制限など）に最適化

...
- **入力対応形式**: 
  - **完全サポート（推奨）**:
    - MP4 (.mp4, .m4v) - H.264/H.265/AV1/AAC
    - MOV (.mov) - QuickTime形式 (同上のコーデック)
  - **実験的サポート**:
    - WebM (.webm) - VP8/VP9/AV1/Opus/Vorbis
    - ⚠️ **注意**: 一部のWebMファイル（特に8K AV1）でデコードエラーが発生する既知の問題があります。MP4/MOV形式の使用を推奨します。
...
### Q: WebMファイルのエンコードが失敗する

A: **WebM形式のサポートは実験的です**。特に8K AV1を含むWebMファイルでデコードエラーが発生する既知の問題があります。以下の対処法をお試しください：

1. **MP4/MOV形式に変換する（推奨）**: FFmpegなどで事前にMP4に変換してください
   ```bash
   ffmpeg -i input.webm -c copy output.mp4
   ```

2. **別のブラウザを試す**: Chrome、Edge、Firefoxなど異なるブラウザで試してください

3. **低解像度で試す**: 8Kではなく4K以下の解像度で試してください

4. **MP4/MOV入力を使用する**: 同じ動画のMP4/MOV版があればそちらをご利用ください

**重要**: WebMサポートは実験的機能であり、安定動作は保証されません。MP4/MOV形式の使用を強く推奨します。
